...
function install_network_manager_conf {
    local pair=$(get_value ovs cidr_mappings)
    local bridge=`echo $pair | cut -f 1 -d ":"`
    local cidr=`echo $pair | cut -f 2 -d ":"`
    local ip=`echo $cidr | cut -f 1 -d "/"`
    local prefix=`echo $cidr | cut -f 2 -d "/"`
    local mask=""
    local full_octets=$(expr $prefix / 8)
    local partial_octet=$(expr $prefix % 8)
